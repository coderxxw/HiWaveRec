        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if update_flag:
                    ...
                    remaining_patience = self.stopping_step - self.cur_step
                    remaining_epochs = remaining_patience * self.eval_step
                    if verbose:
                        self.logger.info(
                            set_color("No improvement", "yellow")
                            + ": patience %d/%d, early stop in %d epoch(s)"
                            % (self.cur_step, self.stopping_step, remaining_epochs)
                        )
